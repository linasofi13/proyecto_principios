...
public class Accesorio {
	...
	public void setNombreAccesorio(String nombre) { //no estoy segura si los métodos set son necesarios en esta clase, ya que los valores ya vienen predefinidos.
		this.nombre = nombre;
	}
	...
	public static void personalizarPlanta(Accesorio[] prendas, Accesorio[] macetas, Accesorio[] fondos, ArrayList<Cuenta> cuentas, ArrayList<Accesorio> accesoriosComprados) {
		...
		if(opcion == 0){ //continuar a la tienda
			...
			if(opcion3 == 1) { //comprar un accesorio prendas
				Accesorio.comprarAccesorioPrendas(prendas, cuentas, accesoriosComprados); //check
			}
				...
				for(int i = 0; i < fondos.length ; i++) { // TO DO
	...
	public static void comprarAccesorioPrendas(Accesorio[] prendas, ArrayList<Cuenta> cuentas, ArrayList<Accesorio> accesoriosComprados) {
		...
		for(int j = 0; j < prendas.length; j++) { // preguntar profe
			for(int i = 0; i < prendas.length ; i++) {
				System.out.println(j+" | " +prendas[i].toString());
				j = i;
			}
		}
		...
		case 9:
			...
			break;
		case 10:
			Accesorio.comprobarPagoPrendas(prendas, cuentas, accesoriosComprados,10);
			break;
		default:
	...
	public static void comprarAccesorioMacetas(Accesorio[] macetas, ArrayList<Cuenta> cuentas, ArrayList<Accesorio> accesoriosComprados) {
		Scanner scan = new Scanner(System.in);
		System.out.println("¿Qué Accesorio deseas comprar? \n");
		for(int j = 0; j < macetas.length; j++) { // preguntar profe
			for(int i = 0; i < macetas.length ; i++) {
				System.out.println(j+" | " + macetas[i].toString());
				j = i;
			}
		}
		int entrada = scan.nextInt();
		...
		case 4:
			...
			break;
		case 5:
			Accesorio.comprobarPagoMacetas(macetas, cuentas, accesoriosComprados,5);
			break;
		default:
			System.out.println("Opción no válida");
			break;
	...
	Scanner scan = new Scanner(System.in);
	System.out.println("¿Qué Accesorio deseas comprar? \n");
	for(int j = 0; j < fondos.length; j++) { // preguntar profe
		for(int i = 0; i < fondos.length ; i++) {
			System.out.println(j+" | " + fondos[i].toString());
			j = i;
		}
	}
	int entrada = scan.nextInt();
	...
	case 4:
		...
		break;
	case 5:
		Accesorio.comprobarPagoFondos(fondos, cuentas, accesoriosComprados,5);
		break;
	default:
		System.out.println("Opción no válida");
		break;
...
public static void visualizarAccesoriosComprados(ArrayList<Accesorio> accesoriosComprados) {
	System.out.println("----Accesorios Comprados:-----");
		System.out.println(accesoriosComprados.toString()); //TO DO
}
}
